...
struct ecefvel {
    // Orbital vel of satellite in Earth Centred Earth Fixed Co-Ordinate System
    public:
    double vx; // vector passing through Greenwich Meridian
    double vy; // orthogonal towards Equator
    double vz; // passes through Celestial North Pole
};
...
struct llapos {
    // Ground-based sub vehicle point in Latitude Longitude Altitude - centred on Earth
    public:
    double lon; // longtitude (rad)
    double lat; // longitude (rad)
    double alt; // altitude above earth (WGS84)
};
